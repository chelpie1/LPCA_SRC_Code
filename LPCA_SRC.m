...
% Load data. Must specify:
    % X_train (m x n_train matrix of training samples)
    % X_test (m x n_test matrix of test samples)
    % lab (training label vector)
    % ulab_GT (ground truth label of test samples)
    
% tot_num = 100; L = 8; noise_level = 0.01;
...
if d_opt == 0
     % Manually specify d_vect as a Lx1 dimensional vector of intrinsic
     % class dimensions:
     d_vect = ones(L,1);
end
...
for j=1:n_te
    ...
    dist_vects_pos = repmat(y,1,n_tr)-X_tr_norm;
    DIST_pos = sqrt(sum(dist_vects_pos.^2,1))';
    
    dist_vects_neg = repmat(y,1,n_tr)+X_tr_norm;
...
ACC_LPCA_SRC = nnz(ulab_LPCA_SRC == ulab_GT)/n_te;
